...
> 🖱️ **[단계별 인터랙티브 다이어그램 보기 →](https://nb3025.github.io/consensus-build/)** — 각 단계를 클릭하며 흐름을 따라가 볼 수 있습니다.

```mermaid
flowchart LR
    P1[📝 기능정의서] --> R1{🔍 리뷰}
    R1 -->|CRITICAL/MAJOR 있음| F1[✏️ 수정]
    F1 --> R1
    R1 -->|PASS| P3[🗂 구현 계획] --> R2{🔍 리뷰}
    R2 -->|CRITICAL/MAJOR 있음| F2[✏️ 수정]
    F2 --> R2
    R2 -->|PASS| P6[⚙️ 코드 구현] --> P7([✅ 완료])
```

> 🔍 **리뷰**는 Agent 3명이 독립 검토 → 합의한 🔴 CRITICAL/MAJOR 이슈를 ✏️ **수정**에 반영 → 그 결과를 **다시 리뷰**합니다. 중대 이슈가 사라지거나 최대 5라운드에 도달하면 다음 단계로 넘어갑니다.

| 단계 | 하는 일 |
|------|---------|
| **Phase 0** | 상태 파일 생성, 과거 학습(`learnings.md`) 로드 |
| **Phase 1** | 15개 섹션을 갖춘 상세 기능정의서 작성 |
| **Phase 2** | Agent 3명 병렬 리뷰 → 합의된 이슈로 문서 수정 → 재리뷰, 중대 이슈가 없을 때까지 반복 (최대 5라운드) |
